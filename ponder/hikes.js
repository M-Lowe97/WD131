...
//annomys function, triggered when the user clicks on the search button
searchButton.addEventListener("click", e => {
    const userInput = document.getElementById("search").value;

    const filteredHikes = hikes.filter(hike => {

        //if(hike.name.toLowerCase().includes(userInput.toLowerCase())) {
        //    return true;
        //} else {
        //    return false;
        //}

        return hike.name.toLowerCase().includes(userInput.toLowerCase()) || 
            hike.description.toLowerCase().includes(userInput.toLowerCase()) ||
            hike.tags.some(tag => tag.toLowerCase().includes(userInput.toLowerCase()));
    })
...
//displays tags related to a hike
function templateTag(tags) {
    return tags.map((tag) => `<button>${tag}</button>`).join(" ");
}

//creates a difficulty meter
...
//creates the template to display the hikes
function renderHike(hike) {
    let html = hikesTemplate(hike);
    hikeContainer.innerHTML += html
}

//function runs on start up, picking a single random hike
function init(){
    hikeContainer.innerHTML = " ";
    renderHike(hikes[randomNumber]);
}
