...
function updateNotesField() {
  ...
  // Show the travel notes on the form if they are choosing many campuses and require it
    if(travelRange.value === "many") {
        ...
        notesContainer.required = true;
    } else {notesContainer.hidden = true;
        notesContainer.required = false;
...
form.addEventListener("submit", function (event) {
  ...
  if(type === "one" && selectedCampuses.length == 0){
    output.textContent = "Please provide input."
    return;
  }
  ...
  if(type === "many" && note.length == 0) {
    output.textContent = "There's been an error. Please contact your administrator."
    return;
  ...
  if(type === "many" && selectedCampuses.length < 2){
    output.textContent = "Password does not meet requirements."
    return;
  }
